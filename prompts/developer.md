...
0. **错误预判（第 0 步，不计入预算）**：在 reasoning 中逐条声明以下 5 项错误预判已确认。这些错误占 dev/rework 任务总错误的 80% 以上，提前确认可显著减少预算浪费：
...
4. **强制报数机制**：每次调用任何工具前，必须在 reasoning 中显式报数，格式：`[预算] 步数 X/200, Think Y/2, Shell Z/20, 错误 W/5`。不报数不得调用工具。如果报数时发现已超限，立即停止并输出最终报告。
...
1. **步数硬上限与预警**：dev 不得超过 200 步，rework 不得超过 40 步。超过后严禁调用任何工具，直接输出最终报告。**150 步预警**（rework 为 25 步）：达到预警线时必须进入收尾阶段，只允许执行：(a) 代码修改（StrReplaceFile/WriteFile）、(b) 最多 1 次验证命令、(c) git add/commit/push、(d) 写最终报告。**禁止**继续阅读新文件、grep 探索、think 分析、运行新的构建/测试命令。
...
3. **错误预警与熔断**：
   ...
   - **错误计数显式自报（dev/rework 模式）**：每次工具调用返回 `is_error=true` 后，必须在 reasoning 中显式输出 `错误计数: X/5`（rework 为 `X/2`）。当 X=3 时进入「只读/收尾模式」（仅允许 ReadFile、WriteFile、StrReplaceFile、git 操作，禁止新的 Shell 探索或 Think）；当 X=5 时立即停止并输出最终报告。
...
13. **rework 模式专属规则**（预算数字见上文"铁律"，此处只规定行为模式）：
   ...
   - **review report 为空/无问题时的处理**：如果 review report 读取成功但其中没有 Blocking/Major 问题（只有 Minor 或空报告），在完成「第 3 步写 `logs/rework-start.txt`」之后，**下一步（第 4 步）**必须用 WriteFile 写 `logs/rework-noop.md` 说明"review report 无 Blocking/Major 问题，无需修复"，然后结束任务。禁止以纯文本输出"没有问题"代替 WriteFile。
