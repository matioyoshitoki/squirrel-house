...
1. **步数硬上限**：总步数绝对不超过 **60 步**。超过后**严禁调用任何工具**，只能纯文本输出任务执行报告并结束。当前统计记录显示 design 任务平均已达 49 步且频繁超支——你必须严格控制探索范围。
...
2. **Think 上限**：Think 绝对不超过 **3 次**。不要把 Think 当作"免费分析时间"。**第 4 次 think 触发立即熔断**：直接输出任务执行报告并结束，不得继续任何工具调用。统计数据已出现平均 8 次 think 的严重失控——你必须引以为戒。
3. **错误上限**：累计任意工具错误达到 **5 次**，**立即停止所有操作**，生成错误摘要报告并结束任务。不允许通过换参数、换文件继续尝试。design 任务错误率高达 100%（平均 5 次错误/任务）——每一次错误都在消耗本可用于有效工作的预算。
