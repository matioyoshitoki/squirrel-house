...
1. **步数硬上限**：总步数绝对不超过 **60 步**。超过后**严禁调用任何工具**，唯一允许的动作是纯文本输出任务执行报告并结束，不得有任何"再确认一下"的拖延。最新统计：design 任务平均 45 步，最高 79 步——预算已极度紧张，必须立即停止一切无目的试探。
...
2. **Think 上限**：Think 绝对不超过 **3 次**。当前实际平均 think 10.8 次，严重违反此限制。Think 只用于：1) 架构决策；2) 错误恢复策略选择；3) 最终报告总结。**第 3 次 think 后立即熔断**：输出任务执行报告并结束，不得继续任何工具调用。
3. **错误上限与禁止重试**：累计任意工具错误达到 **3 次**时必须进入收尾模式（只允许完成已计划的 WriteFile 和 git 操作）；达到 **5 次**必须**立即停止所有操作**，生成错误摘要报告并结束任务。最新统计：design 任务平均 5 次错误/任务，累计 20 个错误——每次错误都在浪费有效工作预算。**禁止重试**：同一操作（如同一文件的 ReadFile、同一命令的 Shell）失败后，禁止换参数、换路径重试，必须记录错误并跳过。
4. **git status 上限**：全任务最多调用 **3 次** `git status`。
5. **Shell 预算上限**：**所有 Shell 工具调用**总计不得超过 **20 次**（含 git 操作、测试、构建、grep、find、cat、test 等）。当前实际平均 31.5 次，严重超标。每次执行 Shell 前默念"当前第 N 次 Shell，限额 20 次"。达到 15 次时预警，达到 20 次后只允许 git commit/push（如有未提交修改）然后立即结束。
...
12. **错误预判（不计入步数）**：design 任务的常见错误来源为：
...
13. **快速开始（第 1-3 步必须完成）**：
    ...
    3. **设置待办清单**：使用 `SetTodoList` 列出本次任务要完成的 3-5 个具体事项，**并显式包含四项计数器："Think 预算: 0/3"、"步数预算: 0/60"、"Shell 预算: 0/20"、"错误: 0/5"**。每次工具调用后必须更新这些计数器，使预算消耗显性化。
